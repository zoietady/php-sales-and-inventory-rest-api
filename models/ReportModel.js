...
Report.getProductSalesData = result => {
  ...
  let query2 = `select productinformationtable.product_id, productinformationtable.product_name, productinformationtable.product_group,MIN(date_time) as "start_of_week",WEEK(date_time) as "week", sum(ROUND(quantity_sold * productinformationtable.product_price, 2)) as "sales_revenue", sum(quantity_sold) as "volume_sold"
  from productinformationtable, salesordertable 
  where productinformationtable.product_id = salesordertable.product_id
  ...
  let query3 = `SELECT productinventorytable.product_id, productinventorytable.current_stock, productinventorytable.date_time from productinventorytable,(SELECT product_id, current_stock, max(date_time) as latestUpdateDate from productinventorytable GROUP BY product_id) max_product WHERE productinventorytable.product_id = max_product.product_id AND productinventorytable.date_time = max_product.latestUpdateDate;`
