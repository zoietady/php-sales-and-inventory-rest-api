...
Inventory.getAll = result => {
  connection.query("SELECT productinformationtable.*, current_stock,max_stock_capacity, date_time FROM productinformationtable, productinventorytable WHERE productinformationtable.product_id = productinventorytable.product_id GROUP BY product_id;", (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
